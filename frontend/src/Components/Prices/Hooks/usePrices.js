...
const usePrices = () => {
    ...
    const getAdded_tokens = async () => {
        if (selectedUser) {
            ...
            const res = await axios.get('http://localhost:4000/users/addedtoken', axiosConfig)
    ...
    const getFavourites_tokens = async () => {
        if (selectedUser) {
            ...
            const res = await axios.get('http://localhost:4000/users/favouritetoken/', axiosConfig)
    ...
    const newAdded_token = async () => {
        if (selectedUser) {
            ...
            try {
                ...
                await axios.put('http://localhost:4000/users/addedtoken', NewToken, axiosConfig)
    ...
    const deleteAdded_token = async (tokenid, tokenName) => {
        if (selectedUser) {
            ...
            if (opcion === true) {
                ...
                await axios.delete('http://localhost:4000/users/addedtoken/' + tokenid, axiosConfig)
                window.location.href = '/'
            }
        }
    }
    ...
    const deletetokenFavourite = async (tokenid) => {
        if (selectedUser) {
            ...
            await axios.delete('http://localhost:4000/users/favouritetoken/' + tokenid, axiosConfig)
    ...
    const addtokenFavourite = async (tokenid) => {
        if (selectedUser) {
            try {
                ...
                await axios.put('http://localhost:4000/users/favouritetoken', NewTokenFavourite, axiosConfig)
                setIsFavourite(true)
            } catch (error) {
                alert(error.response.data.mensaje)
